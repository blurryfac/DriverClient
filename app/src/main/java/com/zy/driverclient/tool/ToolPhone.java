package com.zy.driverclient.tool;
...
public class ToolPhone {
	...
	/**
	 * 直接呼叫指定的号�?(�?�?<uses-permission android:name="android.permission.CALL_PHONE"/>权限)
	 * @param mContext 上下文Context
	 * @param phoneNumber �?要呼叫的手机号码
	 */
		...
		mContext.registerReceiver(new BroadcastReceiver() {
		    ...
		    public void onReceive(Context _context, Intent _intent) {
		        ...
		        case Activity.RESULT_OK:
		            Toast.makeText(mContext,"短信发�?�成�?", Toast.LENGTH_SHORT).show();
		            break;
	 ...
	 * 获取选择的联系人的手机号�?
	 * @param mContext 上下�?
	 * @param resultCode 请求返回Result状�?�区分代�?
	...
	public static String getChoosedPhoneNumber(Activity mContext,int resultCode,Intent data) {
		...
		if (Activity.RESULT_OK == resultCode) 
		{
			...
			if (phoneNum > 0) {
				// 获得联系人的ID�?
				int idColumn = mCursor.getColumnIndex(BaseColumns._ID);
	...
	public static Bitmap getChoosedImage(Activity mContext,Intent data){
        ...
        // 外界的程序访问ContentProvider�?提供数据 可以通过ContentResolver接口
        ContentResolver resolver = mContext.getContentResolver();

        // 此处的用于判断接收的Activity是不是你想要的那�?
        try {
	...
	public static void openPDFFile(Context mContext, String filePath) {
		try {
			...
			Toast.makeText(mContext, "未检测到可打�?PDF相关软件", Toast.LENGTH_SHORT)
					.show();
		}
	}
	...
	public static void openWordFile(Context mContext, String filePath) {
		try {
			...
			Toast.makeText(mContext, "未检测到可打�?Word文档相关软件", Toast.LENGTH_SHORT)
					.show();
		}
	}
	...
	public static void openOfficeByWPS(Context mContext, String filePath){
		...
		try {
			...
			File file = new File(filePath);
			if (!file.exists()) {
				Toast.makeText(mContext, filePath+"文件路径不存�?", Toast.LENGTH_SHORT).show();
				return;
			}
			
			//�?查是否安装WPS
			String wpsPackageEng = "cn.wps.moffice_eng";//普�?�版与英文版�?�?
//			String wpsActivity = "cn.wps.moffice.documentmanager.PreStartActivity";
			String wpsActivity2 = "cn.wps.moffice.documentmanager.PreStartActivity2";//默认第三方程序启�?
