package com.zy.driverclient.receiver;
...
public class MyReceiver extends BroadcastReceiver {
    ...
    private void processCustomMessage(Context context, Bundle bundle) {
        if (DriverOrderActivity.isForeground) {
            //String message = bundle.getString(JPushInterface.EXTRA_TITLE);
            String extras = bundle.getString(JPushInterface.EXTRA_EXTRA);
            Log.i("----------", extras);
            //Log.i("------==----", message);
            Intent msgIntent = new Intent(DriverOrderActivity.MESSAGE_RECEIVED_ACTION);
            //msgIntent.putExtra(DriverOrderActivity.KEY_MESSAGE, message);
            if (!ExampleUtil.isEmpty(extras)) {
                try {
                    JSONObject extraJson = new JSONObject(extras);

                    if (null != extraJson && extraJson.length() > 0) {
                        msgIntent.putExtra(DriverOrderActivity.KEY_EXTRAS, extras);
                    }
                } catch (JSONException e) {
